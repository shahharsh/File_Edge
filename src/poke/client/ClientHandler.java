	...
	public void handleMessage(eye.Comm.Response msg) {
		for (String id : listeners.keySet()) {
			...
			if(msg.getHeader().getRoutingId().getNumber()==20){
				//reply for DOADD
				System.out.println("Message from DOCADD is >>>  "+msg.getHeader().getReplyMsg());
			}
			if(msg.getHeader().getRoutingId().getNumber()==21){
				//reply for DOCFIND
				System.out.println("Message from DOCFIND is >>>  "+msg.getHeader().getReplyMsg());
			}
			if(msg.getHeader().getRoutingId().getNumber()==22){
				//reply for DOCUPDATE
				System.out.println("Message from DOCUPDATE is >>>  "+msg.getHeader().getReplyMsg());
			}
			if(msg.getHeader().getRoutingId().getNumber()==23){
				//reply for DOCREMOVE
				System.out.println("Message from DOCREMOVE is >>>  "+msg.getHeader().getReplyMsg());
				
			}if(msg.getHeader().getRoutingId().getNumber()==24){
				//reply for DOCHANDSHAKE
				System.out.println("Message from DOCHANDSHAKE is >>>  "+msg.getHeader().getReplyMsg());
			}
			if(msg.getHeader().getRoutingId().getNumber()==25){
				//reply for DOCQUERY
				System.out.println("Message from DOCQUERY is >>>  "+msg.getHeader().getReplyMsg());
			}
