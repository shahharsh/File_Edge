...
public class ForwardResource implements Resource {
	...
	public Response process(Request request) {
		System.out.println("In ForwardResource Process Method");
		
		String nextNode = determineForwardNode(request);
		System.out.println("In ForwardResource Process Method next node determined is >>>>  "+nextNode);
		

		if (nextNode != null) {
			System.out.println("When nextNode is not null--------------------------------------------");
			Request fwd = ResourceUtil.buildForwardMessage(request, cfg);
			
			// TODO forward the request

			String ipFromClient = cfg.getNearest().getNode(nextNode).getHost();
			System.out.println("In ForwardResource Process Method ipFromClient is >>>>  "+ipFromClient);
			int portFromClient = cfg.getNearest().getNode(nextNode).getPort();
			System.out.println("In ForwardResource Process Method portFromClient is >>>>  "+portFromClient);
			
			//ClientConnection clientConnection = ClientConnection.initConnection(ipFromClient, portFromClient);
			ClientConnection clientConnection = ClientConnection.initConnection(ipFromClient, portFromClient);
			
			System.out.println("Before Calling forward resource");
			clientConnection.forwardResource(request);
	...
	private String determineForwardNode(Request request) {
		...
		if (paths == null || paths.size() == 0) {
			...
			//return nd.getNodeId();
			System.out.println("the nod to send is -----------------------------------------------00000"+nd.getPort()+nd.getNodeId());
			return cfg.getNearest().getNearestNodes().lastKey();
		} else {
			...
			for (RoutingPath rp : paths) {
				for (NodeDesc nd : cfg.getNearest().getNearestNodes().values()) {
					if (!nd.getNodeId().equalsIgnoreCase(rp.getNode()))
						return nd.getNodeId();
						//return cfg.getNearest().getNearestNodes().get(cfg.getNearest().getNearestNodes().firstKey()).getNodeId();
				}
			}
		}
