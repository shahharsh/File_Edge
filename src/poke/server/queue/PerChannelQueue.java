...
import poke.resources.NameSpaceResource;
import poke.resources.PokeResource;
...
public class PerChannelQueue implements ChannelQueue {
	...
	/********************************************************************************************************************/
					...
					if (msg instanceof Request) {
						...
						{
							System.out.println("Yes... rsc is an instance of ForwardResource");
	...
	/********************************************************************************************************************/						
		...
		// Replication code starts here... After queueing response to the client request is being forwarded to the next node.
		/*******************************************************************************************************************/
				// if instance is not an instance of ForwardResource then forward the request to the nearest node.
						
					//if((rsc instanceof ForwardResource))// && !(rsc instanceof NameSpaceResource) && !((rsc instanceof PokeResource)))
						//{
					/*if(req.getHeader().getRoutingId().getNumber() == 20){	
						System.out.println("After queueing response forwarding request to nearest neighbour.");
						Response response = null; 
						Resource forwardResource = ResourceFactory.getInstance().getForwardResouceInstance(req.getHeader());
	
					// First do the docAddHandshake and then go for docAdd()	
					// If Handshake responds failure drop the message. Do nothing	
						req = ClientUtil.putHandshakeinRequest(req);
							response = forwardResource.process(req);
							System.out.println("Reply status in handshake >>  "+response.getHeader().getReplyCode());
						if(response.getHeader().getReplyCode().equals(ReplyStatus.FAILURE)){
							System.out.println("Namespace and file is not present forwarding the file to the nearest node");
							req = ClientUtil.putAddinRequest(req);
							forwardResource.process(req);
						}						else{
							System.out.println("Namespace and file is present dropping the request");
