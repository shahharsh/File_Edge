...
public class ResourceFactory {
	...
	/********************************************************/
	// Code for obtaining a forward resource
	
	public Resource getForwardResouceInstance(Header header){
		System.out.println("In getForwardResouceInstance function of ResourceFactory");
		Resource resource = null;
		try {
			resource = (Resource) Beans.instantiate(this.getClass().getClassLoader(), cfg.getServer().getProperty("forward"));
			if(resource instanceof ForwardResource){
				((ForwardResource) resource).setCfg(cfg);
			}
	...
	public Resource resourceInstance(Header header) {
		...
		if (header.hasToNode()) {
			...
			if (iam.equalsIgnoreCase(header.getToNode()))
				...
				try {
					// Getting an instance of ForwardResource in case node id specified is other than current
					
					Resource resource = (Resource) Beans.instantiate(this.getClass().getClassLoader(), cfg.getServer().getProperty("forward"));
					if(resource instanceof ForwardResource){
						((ForwardResource) resource).setCfg(cfg);
					}
					return resource;
